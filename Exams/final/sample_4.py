...
def is_word_word_ladder(word_1, word_2, candidate_ladder):
    ...
    path = []
    for case in candidate_ladder:
        if case not in lexicon:
            return False
    if word_2 == word_1 and word_1 in candidate_ladder:
        return True
    if word_1 not in candidate_ladder or word_2 not in candidate_ladder:
        return False
    ...
    while len(queue) != 0:
        path = queue.pop()
        index = path[-1]
        for case in closest_words[index]:
            if case == word_2:
                if not solution or len(solution[-1]) == len(path)+1:
                    solution.append(path + [case])
            if not solution and case not in path:
                queue.appendleft(path + [case])
